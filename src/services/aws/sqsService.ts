...
export const receiveMessagesFromQueue = async (loggedInUserId: string): Promise<any[] | null> => {
  try {
    ...
    // Filter the messages by 'userId' to process only events belonging to the logged-in user
    const filteredMessages = (Messages || []).filter((message) => {
      const userId = message.MessageAttributes?.userId?.StringValue;
      return userId === loggedInUserId; // Only returns messages whose 'userId' matches
    });
